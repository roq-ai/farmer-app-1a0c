...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const allowed = await prisma.farm
    .withAuthorization({
      roqUserId,
  ...
  async function getFarmById() {
    const data = await prisma.farm.findFirst(convertQueryToPrismaUtil(req.query, 'farm'));
    return res.status(200).json(data);
  }

  async function updateFarmById() {
