...
type ColumnType = ColumnDef<FarmInterface, unknown>;
...
export function FarmListPage(props: FarmListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getFarms({
        relations: ['user', 'financial_statement.count', 'purchase.count', 'sale.count', 'visit.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['description.contains', 'location.contains', 'type.contains', 'name.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<FarmInterface>>(
    () => `/farms?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: FarmInterface) => {
    if (hasAccess('farm', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/farms/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('financial_statement', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'financial_statement',
          header: 'Financial Statement',
          accessorKey: 'financial_statement',
          cell: ({ row: { original: record } }: any) => record?._count?.financial_statement,
        }
      : null,
    ...
    hasAccess('sale', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('visit', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'visit',
          header: 'Visit',
          accessorKey: 'visit',
          cell: ({ row: { original: record } }: any) => record?._count?.visit,
        }
      : null,
