...
import { getFinancialStatementById } from 'apiSdk/financial-statements';
import { FinancialStatementInterface } from 'interfaces/financial-statement';

function FinancialStatementViewPage() {
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Financial Statement Details',
              isCurrent: true,
            },
          ]}
        />
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('financial_statement', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/financial-statements/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Contract Id" text={data?.contract_id} />

                <FormListItem label="Total Income" text={data?.total_income} />

                <FormListItem label="Total Expense" text={data?.total_expense} />

                <FormListItem label="Net Income" text={data?.net_income} />

                <FormListItem
                  label="Statement Date"
                  text={
                    data?.statement_date
                      ? format(parseISO(data?.statement_date as unknown as string), 'dd-MM-yyyy')
                      : ''
                  }
                />
