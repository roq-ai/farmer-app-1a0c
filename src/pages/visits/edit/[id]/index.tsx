...
import { getVisitById, updateVisitById } from 'apiSdk/visits';
...
function VisitEditPage() {
  ...
  const handleSubmit = async (values: VisitInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateVisitById(id, values);
      mutate(updated);
      resetForm();
      router.push('/visits');
    } catch (error: any) {
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          <TextInput
            error={formik.errors.salesperson_name}
            label={'Salesperson Name'}
            props={{
              name: 'salesperson_name',
              placeholder: 'Salesperson Name',
              value: formik.values?.salesperson_name,
