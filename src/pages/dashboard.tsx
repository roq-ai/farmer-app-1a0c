...
import { getFinancialStatements } from 'apiSdk/financial-statements';
import { FinancialStatementInterface } from 'interfaces/financial-statement';
import { getVisits } from 'apiSdk/visits';
import { VisitInterface } from 'interfaces/visit';
import { getSales } from 'apiSdk/sales';
import { SaleInterface } from 'interfaces/sale';

import { VisitListPage as ShowCaseList } from 'pages/visits';

function DashboardPage() {
  const { session } = useSession();

  const financialStatementStat = useSWR<PaginatedInterface<FinancialStatementInterface>>(
    () => `/financial-statements`,
    () => getFinancialStatements({ limit: 1, offset: 0 }),
  );

  const visitStat = useSWR<PaginatedInterface<VisitInterface>>(
    () => `/visits`,
    () => getVisits({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: financialStatementStat?.data?.totalCount,
      isLoading: financialStatementStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Visits',
      result: visitStat?.data?.totalCount,
      isLoading: visitStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Sales',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`visits`} passHref legacyBehavior>
                  <Button
