...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'farm-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<FarmInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/farms` : null),
    () => getFarms({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Farms',
      path: isTenantUser ? `/farms/view/${data?.[0]?.id}` : '/farms',
      entity: 'farm',
    ...
    { name: 'Visits', path: '/visits', entity: 'visit', service: AccessServiceEnum.PROJECT, icon: FiCalendar },
