...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'purchase.update': {
    roles: ['business-owner', 'farm-manager'],
    key: 'purchase-update',
    tenantPath: ['farm', 'purchase'],
    userPath: [],
  },
  'sale.update': {
    roles: ['business-owner', 'farm-manager'],
    key: 'sale-update',
    tenantPath: ['farm', 'sale'],
    userPath: [],
  },
  'visit.update': {
    roles: ['business-owner', 'farm-manager'],
    key: 'visit-schedule-update',
    tenantPath: ['farm', 'visit'],
    userPath: [],
  },
};
...
const tenantRoles: string[] = ['business-owner', 'farm-manager'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.farm.findFirst({
